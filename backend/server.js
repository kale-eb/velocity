      ...
      for (const tc of toolCalls) {
        try {
          ...
          if (fn === 'get_product_specs') {
            ...
            payload.forEach(p => {
              if (p.analysis) {
                console.log(`       ✅ ${p.url}: ${p.analysis.chunks?.length || 0} segments, ${p.analysis.duration || 0}s`);
              } else {
        ...
        if (chunk.type === 'response.output_item.added') {
          ...
          if (!foundTextContent && !hasStreamedContent && toolOutputs.length > 0 && !calledSuggestScriptChanges) {
            ...
            try {
              ...
              res.write(`data: ${JSON.stringify({
                type: "content",
                content: `I can see your script "${script?.title || 'current script'}" with ${script?.sections?.length || 0} sections. It looks like it's about ${script?.sections?.[0]?.script_text?.substring(0, 50) || 'your video content'}... How can I help you improve it?`
              })}\n\n`);
            }
          }
          break;
